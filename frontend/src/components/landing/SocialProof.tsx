...
const itemVariants = {
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, ease: [0.42, 0, 0.58, 1] as any }, // easeOut bezier curve
  },
};
...
export const SocialProofComponent: React.FC<SocialProofProps> = ({ socialProofContent }) => {
  ...
  return (
    <section id="testimonials" className="py-24 sm:py-32 bg-primary-50/50" aria-labelledby="social-proof-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h2 id="social-proof-heading" className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl lg:text-6xl">
            {sectionTitle}
          </h2>
        </div>
        ...
        {statistics && statistics.length > 0 && (
          <motion.div
            className="mt-20 border-t border-border pt-16 grid grid-cols-1 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.3 }}
            ...
            {statistics.map((stat, index) => {
              ...
              return (
                <motion.div key={index} variants={itemVariants} className="flex flex-col items-center text-center">
                  ...
                  <p className="text-5xl font-extrabold text-foreground">{stat.number}</p>
